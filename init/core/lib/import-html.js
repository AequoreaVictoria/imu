...
function searchRoot(type, file) {
  for (let p in Current.paths) {
    const match = find(
      type,
      path.resolve(process.cwd(), Current.paths[p], file)
    );
...
function handleScript(origin, file, result) {
  ...
  const match =
    searchRelative("script", file, origin) || searchRoot("script", file);
  if (!match) {
    console.error("ERROR: Could not add @script " + file + "!");
    process.exit(1);
  }

  for (const ln of scan(fs.readFileSync(match), match, true, tagName))
    result.push(ln);
  return result;
}
...
function handleImport(origin, file, result) {
  const match =
    searchRelative("import", file, origin) || searchRoot("import", file);
  if (!match) {
...
function scan(buffer, origin, tagged, tagName, newFile) {
  ...
  for (const line of array) {
    if (line.match(importTag))
      result = handleImport(origin, line.match(importTag)[1], result);
    else if (line.match(scriptTag))
      result = handleScript(origin, line.match(scriptTag)[1], result);
    else if (line.match(svgTag))
      result = handleSvg(origin, line.match(svgTag)[1], result);
...
module.exports = function (buffer, origin, options) {
  if (options) {
    ...
    if (options.extensions) {
      if (options.extensions.import)
        Current.extensions.import = options.extensions.import;
      if (options.extensions.script)
        Current.extensions.script = options.extensions.script;
      if (options.extensions.svg)
        Current.extensions.svg = options.extensions.svg;
    }
  }
